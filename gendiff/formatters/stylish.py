...
def stylish_formatter(source, depth=1):
    ...
    for item in source:
        match item['status']:
            case 'nested':
                result.append(f"{INDENTS*2*depth}{item['key']}:"
                              f" {stylish_formatter(item['value'], depth+1)}")
            case 'added':
                result.append(f"{INDENTS*(2*depth-1)}{PLUS}{item['key']}:"
                              f" {display_like_json(item['value'], depth + 1)}")
            case 'deleted':
                result.append(f"{INDENTS*(2*depth-1)}{MINUS}{item['key']}:"
                              f" {display_like_json(item['value'], depth + 1)}")
            case 'unchanged':
                result.append(f"{INDENTS*2*depth}{item['key']}:"
                              f" {display_like_json(item['value'], depth + 1)}")
    ...
    match structure:
        ...
        case dict():
            ...
            for key, value in structure.items():
                result.append(f'{INDENTS*2*depth}{key}:'
